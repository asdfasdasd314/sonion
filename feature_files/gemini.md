...
The Gemini integration sends Sonion food descriptions to a configurable Google AI model through a result-driven server-side agent loop. Gemma 4 31B is the default while allowing local deployments to select another supported model without changing request code.
