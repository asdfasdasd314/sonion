...
Meal copy duplicates a saved meal's full estimate snapshot into the center interpreter as a new draft. The user can change the local date and time, optionally apply a small structured revision, and save the result as a separate history record without mutating the source meal.
...
- Optional refinements reuse the existing structured revision protocol; a revision is not required before saving a copy.
...
- `components/meal-interpreter.tsx` renders the copy draft, optional revision, date/time fields, and POST-only save path.
...
- Added copy-draft helpers, history copy action, interpreter copy mode with optional refinement, and POST-only save so routine meals can be retargeted to a new date and time.
